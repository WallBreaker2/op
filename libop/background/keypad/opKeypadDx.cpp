...
opKeypadDx::opKeypadDx() = default;

opKeypadDx::~opKeypadDx() {
    UnBind();
}

long opKeypadDx::Bind(HWND hwnd, long mode) {
...
long opKeypadDx::UnBind() {
    input_hook_client::UnBind(_hwnd);
    _keys.fill(0);
    return bkkeypad::UnBind();
}

long opKeypadDx::GetKeyState(long vk_code) {
    return valid_vk(vk_code) && _keys[static_cast<size_t>(vk_code)] ? 1 : 0;
}

long opKeypadDx::KeyDown(long vk_code) {
    if (!valid_vk(vk_code))
        return 0;
...
long opKeypadDx::KeyUp(long vk_code) {
    if (!valid_vk(vk_code))
        return 0;
...
long opKeypadDx::WaitKey(long vk_code, unsigned long time_out) {
...
long opKeypadDx::KeyPress(long vk_code) {
