...
bool load_dinput_vtables() {
    ...
    auto directInput8Create = reinterpret_cast<DirectInput8CreateFn *>(query_api("dinput8.dll", "DirectInput8Create"));
...
void hook_win32_key_state() {
    auto getKeyState = reinterpret_cast<void *>(query_api("user32.dll", "GetKeyState"));
    auto getAsyncKeyState = reinterpret_cast<void *>(query_api("user32.dll", "GetAsyncKeyState"));
    auto getKeyboardState = reinterpret_cast<void *>(query_api("user32.dll", "GetKeyboardState"));
...
void hook_win32_cursor() {
    auto setCursor = reinterpret_cast<void *>(query_api("user32.dll", "SetCursor"));
    if (setCursor)
        MH_CreateHook(setCursor, hkSetCursor, &g_setCursorRaw);
}

void hook_raw_input() {
    auto getRawInputData = reinterpret_cast<void *>(query_api("user32.dll", "GetRawInputData"));
    auto getRawInputBuffer = reinterpret_cast<void *>(query_api("user32.dll", "GetRawInputBuffer"));
    auto registerRawInputDevices = reinterpret_cast<void *>(query_api("user32.dll", "RegisterRawInputDevices"));
    auto getRawInputDeviceInfoW = reinterpret_cast<void *>(query_api("user32.dll", "GetRawInputDeviceInfoW"));
    auto getRawInputDeviceInfoA = reinterpret_cast<void *>(query_api("user32.dll", "GetRawInputDeviceInfoA"));
    auto getRawInputDeviceList = reinterpret_cast<void *>(query_api("user32.dll", "GetRawInputDeviceList"));
    auto getRegisteredRawInputDevices =
        reinterpret_cast<void *>(query_api("user32.dll", "GetRegisteredRawInputDevices"));
...
void fill_mouse_state(DWORD size, LPVOID ptr) {
    if (size == sizeof(opMouseState)) {
        memcpy(ptr, &InputHook::m_mouseState, sizeof(opMouseState));
        return;
    }
...
HRESULT __stdcall hkGetDeviceState(IDirectInputDevice8W *device, DWORD size, LPVOID ptr) {
    ...
    if (ptr && (is_mouse || (!known_device && same_vtable(device, g_mouseVtablePtr))) &&
        (size == sizeof(opMouseState) || size == sizeof(DIMOUSESTATE) || size == sizeof(DIMOUSESTATE2))) {
