...
#include "./imageProc/YoloWrapper.h"
...
long libop::SetYoloEngine(const wchar_t *path_of_engine, const wchar_t *dll_name, const wchar_t *argv) {
    string argvs = argv ? _ws2string(argv) : "";
    vector<string> vstr;
    split(argvs, vstr, " ");
    const std::wstring engine = path_of_engine ? path_of_engine : L"";
    const std::wstring dll = dll_name ? dll_name : L"";
    return YoloWrapper::getInstance()->init(engine, dll, vstr) == 0 ? 1 : 0;
}
...
static void build_yolo_json(const vyolo_rec_t &items, std::wstring &retjson) {
...
void libop::YoloDetect(long x1, long y1, long x2, long y2, double conf, double iou, std::wstring &retjson, long *ret) {
    retjson.clear();
    if (ret)
        *ret = 0;
    if (m_context->bkproc.check_bind() && m_context->bkproc.RectConvert(x1, y1, x2, y2)) {
        ...
        vyolo_rec_t res;
        const int n = YoloWrapper::getInstance()->detect(m_context->image_proc._src.pdata, m_context->image_proc._src.width,
...
void libop::YoloDetectFromFile(const wchar_t *file_name, double conf, double iou, std::wstring &retjson, long *ret) {
    retjson.clear();
    if (ret)
        *ret = 0;
    ...
    if (!img.read(fullpath.data()))
        return;
    vyolo_rec_t res;
    const int n = YoloWrapper::getInstance()->detect(img.pdata, img.width, img.height, 4, conf, iou, res);
    if (n < 0)
        return;
    build_yolo_json(res, retjson);
    if (ret)
