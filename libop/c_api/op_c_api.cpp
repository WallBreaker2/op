...
int call_int(op_handle handle, Func &&func) {
    libop *op = handle ? &handle->op : nullptr;
    if (!op)
        return 0;
...
int call_ret(op_handle handle, Func &&func) {
    return call_int(handle, [&](libop &op) {
        long ret = 0;
...
int OP_CALL OpSetPath(op_handle handle, const wchar_t *path) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetPath(safe_text(path), ret); });
}

const wchar_t *OP_CALL OpGetPath(op_handle handle) {
    return call_string(handle, [](libop &op, std::wstring &ret) { op.GetPath(ret); });
}

const wchar_t *OP_CALL OpGetBasePath(op_handle handle) {
    return call_string(handle, [](libop &op, std::wstring &ret) { op.GetBasePath(ret); });
}

int OP_CALL OpGetID(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.GetID(ret); });
}

int OP_CALL OpGetLastError(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.GetLastError(ret); });
}

int OP_CALL OpSetShowErrorMsg(op_handle handle, int show_type) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetShowErrorMsg(show_type, ret); });
}

int OP_CALL OpSleep(op_handle handle, int millseconds) {
    return call_ret(handle, [&](libop &op, long *ret) { op.Sleep(millseconds, ret); });
}

int OP_CALL OpInjectDll(op_handle handle, const wchar_t *process_name, const wchar_t *dll_name) {
    return call_ret(handle, [&](libop &op, long *ret) {
        op.InjectDll(safe_text(process_name), safe_text(dll_name), ret);
    });
}

int OP_CALL OpEnablePicCache(op_handle handle, int enable) {
    return call_ret(handle, [&](libop &op, long *ret) { op.EnablePicCache(enable, ret); });
}

int OP_CALL OpCapturePre(op_handle handle, const wchar_t *file_name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.CapturePre(safe_text(file_name), ret); });
}

int OP_CALL OpSetScreenDataMode(op_handle handle, int mode) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetScreenDataMode(mode, ret); });
}
...
const wchar_t *OP_CALL OpAStarFindPath(op_handle handle, int map_width, int map_height,
                                       ...
                                       int end_y) {
    return call_string(handle, [&](libop &op, std::wstring &ret) {
...
const wchar_t *OP_CALL OpEnumProcess(op_handle handle, const wchar_t *name) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.EnumProcess(safe_text(name), ret); });
}
...
intptr_t OP_CALL OpFindWindow(op_handle handle, const wchar_t *class_name, const wchar_t *title) {
    return call_intptr(handle, [&](libop &op, LONG_PTR *ret) { op.FindWindow(safe_text(class_name), safe_text(title), ret); });
}

intptr_t OP_CALL OpFindWindowByProcess(op_handle handle, const wchar_t *process_name, const wchar_t *class_name,
                                       const wchar_t *title) {
    return call_intptr(handle, [&](libop &op, LONG_PTR *ret) {
...
intptr_t OP_CALL OpGetForegroundFocus(op_handle handle) {
    return call_intptr(handle, [](libop &op, LONG_PTR *ret) { op.GetForegroundFocus(ret); });
}

intptr_t OP_CALL OpGetForegroundWindow(op_handle handle) {
    return call_intptr(handle, [](libop &op, LONG_PTR *ret) { op.GetForegroundWindow(ret); });
}

intptr_t OP_CALL OpGetMousePointWindow(op_handle handle) {
    return call_intptr(handle, [](libop &op, LONG_PTR *ret) { op.GetMousePointWindow(ret); });
}

intptr_t OP_CALL OpGetPointWindow(op_handle handle, int x, int y) {
    return call_intptr(handle, [&](libop &op, LONG_PTR *ret) { op.GetPointWindow(x, y, ret); });
}

const wchar_t *OP_CALL OpGetProcessInfo(op_handle handle, int pid) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.GetProcessInfo(pid, ret); });
}

intptr_t OP_CALL OpGetSpecialWindow(op_handle handle, int flag) {
    return call_intptr(handle, [&](libop &op, LONG_PTR *ret) { op.GetSpecialWindow(flag, ret); });
}
...
int OP_CALL OpGetWindowProcessId(op_handle handle, intptr_t hwnd) {
    return call_ret(handle, [&](libop &op, long *ret) { op.GetWindowProcessId(static_cast<LONG_PTR>(hwnd), ret); });
}
...
int OP_CALL OpGetWindowState(op_handle handle, intptr_t hwnd, int flag) {
    return call_ret(handle, [&](libop &op, long *ret) { op.GetWindowState(static_cast<LONG_PTR>(hwnd), flag, ret); });
}
...
int OP_CALL OpMoveWindow(op_handle handle, intptr_t hwnd, int x, int y) {
    return call_ret(handle, [&](libop &op, long *ret) { op.MoveWindow(static_cast<LONG_PTR>(hwnd), x, y, ret); });
}
...
int OP_CALL OpSendPaste(op_handle handle, intptr_t hwnd) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SendPaste(static_cast<LONG_PTR>(hwnd), ret); });
}
...
int OP_CALL OpRunApp(op_handle handle, const wchar_t *cmdline, int mode, uint32_t *pid) {
    return call_ret(handle, [&](libop &op, long *ret) {
        ...
        if (pid)
            *pid = static_cast<uint32_t>(local_pid);
    });
}

int OP_CALL OpWinExec(op_handle handle, const wchar_t *cmdline, int cmdshow) {
    return call_ret(handle, [&](libop &op, long *ret) { op.WinExec(safe_text(cmdline), cmdshow, ret); });
}

const wchar_t *OP_CALL OpGetCmdStr(op_handle handle, const wchar_t *cmd, int millseconds) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.GetCmdStr(safe_text(cmd), millseconds, ret); });
}

int OP_CALL OpSetClipboard(op_handle handle, const wchar_t *str) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetClipboard(safe_text(str), ret); });
}

const wchar_t *OP_CALL OpGetClipboard(op_handle handle) {
    return call_string(handle, [](libop &op, std::wstring &ret) { op.GetClipboard(ret); });
}

int OP_CALL OpDelay(op_handle handle, int mis) {
    return call_ret(handle, [&](libop &op, long *ret) { op.Delay(mis, ret); });
}

int OP_CALL OpDelays(op_handle handle, int mis_min, int mis_max) {
    return call_ret(handle, [&](libop &op, long *ret) { op.Delays(mis_min, mis_max, ret); });
}
...
int OP_CALL OpUnBindWindow(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.UnBindWindow(ret); });
}
...
intptr_t OP_CALL OpGetBindWindow(op_handle handle) {
    return call_intptr(handle, [](libop &op, LONG_PTR *ret) { op.GetBindWindow(ret); });
}

int OP_CALL OpIsBind(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.IsBind(ret); });
}
...
const wchar_t *OP_CALL OpGetCursorShape(op_handle handle) {
    return call_string(handle, [](libop &op, std::wstring &ret) { op.GetCursorShape(ret); });
}

int OP_CALL OpMoveR(op_handle handle, int x, int y) {
    return call_ret(handle, [&](libop &op, long *ret) { op.MoveR(x, y, ret); });
}

int OP_CALL OpMoveTo(op_handle handle, int x, int y) {
    return call_ret(handle, [&](libop &op, long *ret) { op.MoveTo(x, y, ret); });
}

const wchar_t *OP_CALL OpMoveToEx(op_handle handle, int x, int y, int w, int h) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.MoveToEx(x, y, w, h, ret); });
}

#define OP_MOUSE_RET(name, method) \
    int OP_CALL name(op_handle handle) { return call_ret(handle, [](libop &op, long *ret) { op.method(ret); }); }
...
int OP_CALL OpSetMouseDelay(op_handle handle, const wchar_t *type, int delay) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetMouseDelay(safe_text(type), delay, ret); });
}

// Keyboard

int OP_CALL OpGetKeyState(op_handle handle, int vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.GetKeyState(vk_code, ret); });
}

int OP_CALL OpKeyDown(op_handle handle, int vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyDown(vk_code, ret); });
}

int OP_CALL OpKeyDownChar(op_handle handle, const wchar_t *vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyDownChar(safe_text(vk_code), ret); });
}

int OP_CALL OpKeyUp(op_handle handle, int vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyUp(vk_code, ret); });
}

int OP_CALL OpKeyUpChar(op_handle handle, const wchar_t *vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyUpChar(safe_text(vk_code), ret); });
}

int OP_CALL OpWaitKey(op_handle handle, int vk_code, int time_out) {
    return call_ret(handle, [&](libop &op, long *ret) { op.WaitKey(vk_code, time_out, ret); });
}

int OP_CALL OpKeyPress(op_handle handle, int vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyPress(vk_code, ret); });
}

int OP_CALL OpKeyPressChar(op_handle handle, const wchar_t *vk_code) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyPressChar(safe_text(vk_code), ret); });
}

int OP_CALL OpSetKeypadDelay(op_handle handle, const wchar_t *type, int delay) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetKeypadDelay(safe_text(type), delay, ret); });
}

int OP_CALL OpKeyPressStr(op_handle handle, const wchar_t *key_str, int delay) {
    return call_ret(handle, [&](libop &op, long *ret) { op.KeyPressStr(safe_text(key_str), delay, ret); });
}

// Image and color

int OP_CALL OpCapture(op_handle handle, int x1, int y1, int x2, int y2, const wchar_t *file_name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.Capture(x1, y1, x2, y2, safe_text(file_name), ret); });
}

int OP_CALL OpCmpColor(op_handle handle, int x, int y, const wchar_t *color, double sim) {
    return call_ret(handle, [&](libop &op, long *ret) { op.CmpColor(x, y, safe_text(color), sim, ret); });
}
...
const wchar_t *OP_CALL OpGetColor(op_handle handle, int x, int y) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.GetColor(x, y, ret); });
}

int OP_CALL OpGetColorNum(op_handle handle, int x1, int y1, int x2, int y2, const wchar_t *color, double sim) {
    return call_ret(handle, [&](libop &op, long *ret) { op.GetColorNum(x1, y1, x2, y2, safe_text(color), sim, ret); });
}

int OP_CALL OpSetDisplayInput(op_handle handle, const wchar_t *mode) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetDisplayInput(safe_text(mode), ret); });
}

int OP_CALL OpLoadPic(op_handle handle, const wchar_t *file_name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.LoadPic(safe_text(file_name), ret); });
}

int OP_CALL OpFreePic(op_handle handle, const wchar_t *file_name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.FreePic(safe_text(file_name), ret); });
}

int OP_CALL OpLoadMemPic(op_handle handle, const wchar_t *file_name, void *data, int size) {
    return call_ret(handle, [&](libop &op, long *ret) { op.LoadMemPic(safe_text(file_name), data, size, ret); });
}
...
const wchar_t *OP_CALL OpMatchPicName(op_handle handle, const wchar_t *pic_name) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.MatchPicName(safe_text(pic_name), ret); });
}

// OpenCV

int OP_CALL OpCvLoadTemplate(op_handle handle, const wchar_t *name, const wchar_t *file_path) {
    return call_ret(handle, [&](libop &op, long *ret) { op.CvLoadTemplate(safe_text(name), safe_text(file_path), ret); });
}
...
int OP_CALL OpCvRemoveTemplate(op_handle handle, const wchar_t *name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.CvRemoveTemplate(safe_text(name), ret); });
}

int OP_CALL OpCvRemoveAllTemplates(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.CvRemoveAllTemplates(ret); });
}

int OP_CALL OpCvHasTemplate(op_handle handle, const wchar_t *name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.CvHasTemplate(safe_text(name), ret); });
}

int OP_CALL OpCvGetTemplateCount(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.CvGetTemplateCount(ret); });
}

const wchar_t *OP_CALL OpCvGetAllTemplateNames(op_handle handle) {
    return call_string(handle, [](libop &op, std::wstring &ret) { op.CvGetAllTemplateNames(ret); });
}

const wchar_t *OP_CALL OpCvGetOpenCvVersion(op_handle handle) {
    return call_string(handle, [](libop &op, std::wstring &ret) { op.CvGetOpenCvVersion(ret); });
}

int OP_CALL OpCvLoadTemplateList(op_handle handle, const wchar_t *template_list) {
    return call_ret(handle, [&](libop &op, long *ret) { op.CvLoadTemplateList(safe_text(template_list), ret); });
}

#define OP_CV_FILE_RET(name, method) \
    int OP_CALL name(op_handle handle, const wchar_t *src_file, const wchar_t *dst_file) { \
        return call_ret(handle, [&](libop &op, long *ret) { op.method(safe_text(src_file), safe_text(dst_file), ret); }); \
    }
...
const wchar_t *OP_CALL OpCvConnectedComponents(op_handle handle, const wchar_t *src_file, double min_area) {
    return call_string(handle, [&](libop &op, std::wstring &json) {
        long ret = 0;
...
int OP_CALL OpSetDict(op_handle handle, int idx, const wchar_t *file_name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetDict(idx, safe_text(file_name), ret); });
}

const wchar_t *OP_CALL OpGetDict(op_handle handle, int idx, int font_index) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.GetDict(idx, font_index, ret); });
}

int OP_CALL OpSetMemDict(op_handle handle, int idx, const wchar_t *data, int size) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SetMemDict(idx, safe_text(data), size, ret); });
}

int OP_CALL OpUseDict(op_handle handle, int idx) {
    return call_ret(handle, [&](libop &op, long *ret) { op.UseDict(idx, ret); });
}

int OP_CALL OpAddDict(op_handle handle, int idx, const wchar_t *dict_info) {
    return call_ret(handle, [&](libop &op, long *ret) { op.AddDict(idx, safe_text(dict_info), ret); });
}

int OP_CALL OpSaveDict(op_handle handle, int idx, const wchar_t *file_name) {
    return call_ret(handle, [&](libop &op, long *ret) { op.SaveDict(idx, safe_text(file_name), ret); });
}

int OP_CALL OpClearDict(op_handle handle, int idx) {
    return call_ret(handle, [&](libop &op, long *ret) { op.ClearDict(idx, ret); });
}

int OP_CALL OpGetDictCount(op_handle handle, int idx) {
    return call_ret(handle, [&](libop &op, long *ret) { op.GetDictCount(idx, ret); });
}

int OP_CALL OpGetNowDict(op_handle handle) {
    return call_ret(handle, [](libop &op, long *ret) { op.GetNowDict(ret); });
}
...
int OP_CALL OpGetWordResultCount(op_handle handle, const wchar_t *result) {
    return call_ret(handle, [&](libop &op, long *ret) { op.GetWordResultCount(safe_text(result), ret); });
}
...
const wchar_t *OP_CALL OpOcr(op_handle handle, int x1, int y1, int x2, int y2, const wchar_t *color, double sim) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.Ocr(x1, y1, x2, y2, safe_text(color), sim, ret); });
}

const wchar_t *OP_CALL OpOcrEx(op_handle handle, int x1, int y1, int x2, int y2, const wchar_t *color, double sim) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.OcrEx(x1, y1, x2, y2, safe_text(color), sim, ret); });
}
...
const wchar_t *OP_CALL OpOcrAuto(op_handle handle, int x1, int y1, int x2, int y2, double sim) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.OcrAuto(x1, y1, x2, y2, sim, ret); });
}
...
const wchar_t *OP_CALL OpOcrAutoFromFile(op_handle handle, const wchar_t *file_name, double sim) {
    return call_string(handle, [&](libop &op, std::wstring &ret) { op.OcrAutoFromFile(safe_text(file_name), sim, ret); });
}
...
int OP_CALL OpReadInt(op_handle handle, intptr_t hwnd, const wchar_t *address, int type, int64_t *value) {
    ...
    return call_memory(handle, hwnd, [&](MemoryEx &mem, HWND target) {
